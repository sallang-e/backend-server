...
class CycleRentAcceptanceTest extends AcceptanceTest {
    ...
    void 이미_다른_사람이_대여중인_살랑이를_대여하면_에러_코드_1을_반환한다() {
        final String user1AccessToken = 회원_가입_요청하고_액세스_토큰_반환(new UserJoinRequest("깃짱", "KAKAO", 123L, 2));
        final String user2AccessToken = 회원_가입_요청하고_액세스_토큰_반환(new UserJoinRequest("훈짱", "KAKAO", 124L, 2));
    ...
    void 살랑이_사용_가능_회수가_없는_유저가_살랑이를_대여하면_에러_코드_3을_반환한다() {
        final String accessToken = 회원_가입_요청하고_액세스_토큰_반환(new UserJoinRequest("깃짱", "KAKAO", 123L, 0));
        final Long cycleId = 살랑이_생성_요청하고_아이디_반환();
